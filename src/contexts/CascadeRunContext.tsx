...
export interface CascadeRunContextValue {
  // State
  isRunning: boolean;
  isPaused: boolean;
  ...
  error: string | null;
  errorPrompt: ErrorPromptData | null;
  ...
  markPromptComplete: (promptRowId: string, promptName: string, response: unknown) => void;
  ...
  isCancelled: () => boolean;
  checkPaused: () => boolean;
  showError: (promptData: ErrorPromptData, errorMessage: string) => Promise<ErrorAction>;
  ...
  setSkipAllPreviews: React.Dispatch<React.SetStateAction<boolean>>;
...
export const CascadeRunProvider = ({ children }: CascadeRunProviderProps) => {
  ...
  const [errorPrompt, setErrorPrompt] = useState<ErrorPromptData | null>(null);
  ...
  const startCascade = useCallback((levels: number, promptCount: number, skippedCount = 0): void => {
    setIsRunning(true);
    setIsPaused(false);
    setCurrentLevel(0);
  ...
  const markPromptSkipped = useCallback((promptRowId: string, promptName: string): void => {
    setSkippedPrompts(prev => [...prev, { promptRowId, promptName }]);
  }, []);

  const updateProgress = useCallback((level: number, promptName: string, promptIndex: number, promptRowId: string | null = null): void => {
  ...
  const markPromptComplete = useCallback((promptRowId: string, promptName: string, response: unknown): void => {
    setCompletedPrompts(prev => [...prev, { promptRowId, promptName, response }]);
  }, []);

  const completeCascade = useCallback((): void => {
    setIsRunning(false);
    setIsPaused(false);
    setCurrentPromptName('');
    setCurrentPromptRowId(null);
  ...
  const registerCancelHandler = useCallback((handler: () => Promise<void>): (() => void) => {
  ...
  const isCancelled = useCallback((): boolean => cancelRef.current, []);
  const checkPaused = useCallback((): boolean => pauseRef.current, []);

  const showError = useCallback((promptData: ErrorPromptData, errorMessage: string): Promise<ErrorAction> => {
  ...
  const resolveError = useCallback((action: ErrorAction): void => {
    setError(null);
    setErrorPrompt(null);
    if (errorResolverRef.current) {
  ...
  const resolveActionPreview = useCallback((confirmed: boolean): void => {
  ...
  const startSingleRun = useCallback((promptRowId: string): void => {
  ...
  const endSingleRun = useCallback((): void => {
  ...
  const resolveQuestion = useCallback((answer: string | null): void => {
  ...
  const addCollectedQuestionVar = useCallback((name: string, value: string): void => {
    setCollectedQuestionVars(prev => [...prev, { name, value }]);
  }, []);

  const resetQuestionState = useCallback((): void => {
    setPendingQuestion(null);
    setQuestionProgress({ current: 0, max: 10 });
    setCollectedQuestionVars([]);
    if (questionResolverRef.current) {
