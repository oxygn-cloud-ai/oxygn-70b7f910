...
export const ApiCallProvider = ({ children }: ApiCallProviderProps) => {
  ...
  const registerCall = useCallback((): (() => void) => {
    callIdRef.current++;
  ...
  const cancelAllCalls = useCallback((): void => {
    setPendingCallsCount(0);
  }, []);

  const addBackgroundCall = useCallback((callInfo: Omit<BackgroundCall, 'id'>): number => {
    const id = ++callIdRef.current;
    setBackgroundCalls((prev) => [...prev, { id, ...callInfo }]);
    return id;
  }, []);

  const removeBackgroundCall = useCallback((id: number): void => {
  ...
  const requestNavigation = useCallback(
    (destination: string, navigateFn: () => void): boolean => {
