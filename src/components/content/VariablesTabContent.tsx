...
interface AIResponseVariableConfig {
  name: string;
  label: string;
  field: string;
}
...
interface PromptData {
  ...
  last_ai_call_metadata?: Record<string, unknown>;
...
interface VariablesTabContentProps {
  ...
  addVariable?: (name: string, value: string, description: string) => Promise<boolean>;
  updateVariable?: (rowId: string, data: Partial<UserVariable>) => Promise<void>;
  deleteVariable?: (rowId: string) => Promise<void>;
}
...
interface EditablePolicyVariableRowProps {
  varName: string;
  systemVariables: Record<string, string>;
  editingPolicyVar: string | null;
  editPolicyValue: string;
  setEditPolicyValue: (value: string) => void;
  isSavingSystemVar: boolean;
  savingVarName: string | null;
  onStartEdit: (varName: string) => void;
  onSave: () => void;
  onCancel: () => void;
}

interface UserVariableRowProps {
  variable: UserVariable;
  editingId: string | null;
  editValue: string;
  setEditValue: (value: string) => void;
  onStartEdit: (variable: UserVariable) => void;
  onSaveEdit: (rowId: string) => void;
  onCancelEdit: () => void;
  onDelete: (rowId: string) => void;
}

type HierarchyType = 'parent' | 'child' | 'sibling';

interface HierarchyVariableRowProps {
  type: HierarchyType;
  promptName?: string;
  extractedVars?: Record<string, unknown>;
}

type SectionKey = 'system' | 'policy' | 'user' | 'hierarchy' | 'aiResponse';
...
const STATIC_VARIABLE_GROUPS: StaticVariableGroup[] = [
  {
    id: 'datetime',
    label: 'Date & Time',
    icon: Calendar,
...
const AI_RESPONSE_VARIABLES: AIResponseVariableConfig[] = [
...
const SectionHeader = ({ 
  ...
  expanded,
  onToggle 
}: SectionHeaderProps & { expanded: boolean; onToggle: () => void }) => (
  <button
    onClick={onToggle}
...
const SystemVariableRow = ({ varName, value }: SystemVariableRowProps) => {
  const def = (SYSTEM_VARIABLES as Record<string, SystemVariableDefinition>)[varName];
  return (
...
const EditablePolicyVariableRow = ({ 
  varName,
  systemVariables,
  editingPolicyVar,
  editPolicyValue,
  ...
  onSave,
  onCancel
}: EditablePolicyVariableRowProps) => {
  const def = (SYSTEM_VARIABLES as Record<string, SystemVariableDefinition>)[varName];
      ...
      {isEditing ? (
        <>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button 
                onClick={() => onSaveEdit(variable.row_id)}
        ...
        <>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button 
                onClick={() => onDelete(variable.row_id)}
...
const HierarchyVariableRow = ({ type, promptName, extractedVars }: HierarchyVariableRowProps) => {
...
const VariablesTabContent = ({ 
  ...
  const {
    ...
    savingVarName,
  } = useSystemVariables(promptRowId, promptData?.system_variables as Record<string, string> | undefined);
  ...
  const [editingPolicyVar, setEditingPolicyVar] = useState<string | null>(null);
  ...
  const aiMetadata = (promptData?.last_ai_call_metadata || {}) as Record<string, unknown>;
  ...
  const handleAdd = async () => {
    if (!newName.trim() || !addVariable) return;
  ...
  const handleStartEditPolicy = (varName: string) => {
    ...
    <div className="space-y-4">
        ...
        {expandedSections.policy && (
          <div className="space-y-1.5 ml-5">
            {EDITABLE_POLICY_VARIABLES.map(varName => (
              <EditablePolicyVariableRow 
                key={varName} 
                varName={varName}
                systemVariables={systemVariables}
                editingPolicyVar={editingPolicyVar}
                editPolicyValue={editPolicyValue}
                setEditPolicyValue={setEditPolicyValue}
                isSavingSystemVar={isSavingSystemVar}
                savingVarName={savingVarName}
                onStartEdit={handleStartEditPolicy}
                onSave={handleSavePolicyEdit}
      ...
      <div className="bg-surface-container-low rounded-m3-lg p-3 space-y-2">
        <SectionHeader 
          ...
          section="aiResponse"
          expanded={expandedSections.aiResponse}
          onToggle={() => toggleSection('aiResponse')}
        />
              ...
              AI_RESPONSE_VARIABLES.filter(v => aiMetadata[v.field] !== undefined).map(v => (
                ...
                >
                  ...
                  <div className="w-32 shrink-0">
                    <div className="h-7 px-2 flex items-center bg-surface-container-high rounded-m3-sm border border-outline-variant text-body-sm text-on-surface-variant truncate font-mono">
                      {v.name === 'q.response' 
                        ? (String(aiMetadata[v.field] || '')).slice(0, 20) + '...'
