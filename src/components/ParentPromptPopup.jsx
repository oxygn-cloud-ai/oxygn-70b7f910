...
const TreeItem = React.forwardRef(({ item, level, expandedItems, toggleItem, activeItem, setActiveItem, selectedItem }, ref) => {
  ...
  return (
    <AccordionItem value={item.id} className="border-none">
      <AccordionTrigger
        ref={ref}
        className={`py-1 px-2 rounded ${isActive ? 'bg-blue-100' : ''} ${isSelected ? 'bg-yellow-200' : ''}`}
        style={{ paddingLeft: `${level * 16}px` }}
        onClick={() => setActiveItem(item)}
      >
        <div className="flex items-center space-x-1">
          <FileIcon className="h-4 w-4 flex-shrink-0" />
          <span className={`ml-1 text-sm ${isActive ? 'text-blue-600 font-bold' : 'text-gray-600 font-normal'}`}>
            {displayName}
          </span>
        </div>
      </AccordionTrigger>
