    ...
    return items.map((item) => (
      <AccordionItem key={item.id} value={item.id}>
        <AccordionTrigger
          ...
          className={`flex items-center py-1 px-2 rounded ${selectedItem?.id === item.id ? 'bg-blue-100' : ''}`}
          style={{ paddingLeft: `${level * 16}px` }}
          onClick={() => toggleItem(item.id)}
        >
          <div className="flex items-center space-x-1 flex-grow">
            <FileIcon className="h-4 w-4 flex-shrink-0" />
            <span className={`ml-1 text-sm ${selectedItem?.id === item.id ? 'text-blue-600 font-bold' : 'text-gray-600 font-normal'}`}>
              {item.prompt_name || `New Prompt {${level}}`}
            </span>
          </div>
        </AccordionTrigger>
        <AccordionContent>
