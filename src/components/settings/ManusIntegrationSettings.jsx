  ...
  const handleRegisterWebhook = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setConnectionStatus({ success: true, message: 'Webhook registered successfully!' });
        setIsWebhookRegistered(true);
        trackEvent('manus_webhook_registered');
      } else {
        setConnectionStatus({ success: false, message: data?.error || 'Registration failed' });
      }
    } catch (err) {
      setConnectionStatus({ success: false, message: err.message || 'Registration failed' });
    } finally {
      setIsTesting(false);
    }
  };
